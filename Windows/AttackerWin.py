...
import subprocess  # Replacement for AppKit functionality

# Color definitions (unchanged)
c_slayer_monster = "FFCD3E00"
...
# Windows-compatible replacement for AppKit's open_application
def open_application(app_name):
    try:
        # For RuneLite on Windows
        if "runelite" in app_name.lower():
            # Try common installation paths
            paths = [
                os.path.expandvars("%PROGRAMFILES%\\RuneLite\\RuneLite.exe"),
                os.path.expandvars("%LOCALAPPDATA%\\RuneLite\\RuneLite.exe"),
                "C:\\Program Files\\RuneLite\\RuneLite.exe"
            ]
...
def is_color_near_other_color(primary_color, secondary_color, primary_tolerance=30, secondary_tolerance=30,
                              min_blob_size=10):
    screenshot = pyautogui.screenshot()
    img = np.array(screenshot)
...
def print_player_stats(sarabrew=0, supres=0, hp=0, prayer=0, run=0, cannon_status=0, current_state=0,
                       div_ranging_potion=0, sharks=0, antivenom=0, boss=0):
...
def main():
    ...
    try:
        ...
        while True:
            ...
            print_player_stats(sarabrew=sarabrew, supres=supres, hp=hp, prayer=prayer, run=run,
                               cannon_status=cannon_status, current_state=current_state, sharks=sharks,
                               antivenom=antivenom, boss=boss)
            if current_state == "SCAN":
                ...
                if IsColourFound(c_idle, tolerance=25):
                    ...
                    if find_and_click_close_to(c_slayer_monster, proximity_hex=c_player_outline, tolerance=1,
