...
def print_player_stats(chisel=0,staminapot=0, bloodrunes=0, housetp=0, dense_e_b=0, dark_e_b=0, dark_essense=0,
                       hp=0, prayer=0, run=0, current_state=0, boss=0, last_action=0):
    ...
    print(f"__________________________________________________________________________")
def main():
    global current_state, TOTAL_INVENTORY_SPACE, last_action
        ...
        while True:
            ...
            if current_state == "MAIN_HANDLE":
                if dense_e_b >= 26:
                    ...
                    last_action = "MINING"
                    continue
                if dark_e_b >= 26 or dark_essense > 0:
                ...
                if IsColourFound(c_idle, tolerance=25):
                    if last_action == "VERNATE":
                        if IsColourFound(c_running_stage_1, tolerance=20):
                            find_and_click_close_to(c_running_stage_1, proximity_hex=c_player_outline, tolerance=1,proximity_tolerance=2)
                    ...
                    if last_action == "RUN2":
                        if IsColourFound(c_running_stage_3, tolerance=20):
                            find_and_click_close_to(c_running_stage_3, proximity_hex=c_player_outline, tolerance=1, proximity_tolerance=2)
                            last_action = "RUN3"
                            continue

                    if last_action == "RUN3":
                        if IsColourFound(c_blood_altar, tolerance=20):
                            find_and_click_close_to(c_blood_altar, proximity_hex=c_player_outline, tolerance=1, proximity_tolerance=2)
                ...
                if IsColourFound(c_idle, tolerance=25):
                    ...
                    if last_action == "OBST3":
                        if IsColourFound(c_obstacle_1, tolerance=20):
                            if find_and_click_close_to(c_obstacle_1, proximity_hex=c_player_outline, tolerance=1, proximity_tolerance=2):
                                last_action = "OBST1"
                                continue
