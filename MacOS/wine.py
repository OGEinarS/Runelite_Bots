...
def print_player_stats(chisel=0,staminapot=0, bloodrunes=0, housetp=0, dense_e_b=0, dark_e_b=0, dark_essense=0,
                       hp=0, prayer=0, run=0, current_state=0, boss=0, teak=0, string=0, uf_bow=0, f_bow=0, d_leather=0,d_body=0, waterjug=0, grapes=0):
...
def main():
    ...
    try:
        ...
        while True:
            ...
            waterjug = count_items_full_screen(c_water_jug, 1, 50)
            grapes = count_items_full_screen(c_grapes, 1, 50)
            ...
            if current_state == "MAIN_HANDLE":
                if grapes >= 14 or waterjug >=14:
                    current_state = "CRAFT_HANDLE"


                if grapes <= 0 or waterjug <= 0:
                    current_state = "BANK_HANDLE"


            elif current_state == "CRAFT_HANDLE":
                if grapes <= 1 or waterjug <= 1:
                    current_state = "BANK_HANDLE"


                if IsColourFound(c_idle, tolerance=25):
                    find_and_click_center(c_water_jug, tolerance=1)
                    humanpause(0.5)
                    find_and_click_center(c_grapes, tolerance=1)
                    humanpause(1)
                    pyautogui.press("space")


            elif current_state == "BANK_HANDLE":
                if grapes or waterjug > 0:
                    current_state = "CRAFT_HANDLE"
                    pyautogui.press("esc")


                if IsColourFound(c_idle, tolerance=25):
                    find_and_click_close_to(c_bank_b, proximity_hex=c_player_outline, tolerance=1, proximity_tolerance=2)
                    humanpause(0.75)
