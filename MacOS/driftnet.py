...
# Color definitions
color_untagged_fish = "FF00D4FF"
color_ready_net = "e9ea08"
color_full_net = "f30000"

color_nets_inv = "8CD6FF"
color_stamina_inv = "EB88FF"

color_anette = "FFBD16DF"
color_anette_nets = "FF0013FF"

# Innstillinger
MIN_LOG_COUNT = 4
ACTION_DELAY = (0.01, 0.01)
MIN_AREA = 2
TOLERANCE = 12
...
def main():
    ...
    try:
        ...
        while True:
            ...
            nets = count_items_full_screen(color_nets_inv)
            stamina = count_items_full_screen(color_stamina_inv)
            print(f"Inventar.")
            print(f"Nets: {nets}")
            print(f"Stamina potions: {stamina}")


            if current_state == "START":
                if nets < 2:
                    print("Tom - går til bank")
                    current_state = "BANK"
                    continue

                if find_and_click_center(color_full_net, tolerance=5):
                    humanpause(4)
                    pyautogui.press("esc")

                if find_and_click_center(color_ready_net, tolerance=5):
                    humanpause(4)
                    pyautogui.press("esc")

                if find_and_click_center(color_untagged_fish, tolerance=10):
                    humanpause(2)
                    current_state = "START"
                    continue
                else:
                ...
                if find_and_click_center(color_anette, tolerance=1):
                    humanpause(3)
                if find_and_click_center(color_anette_nets, tolerance=1):
                    humanpause(0.33)
                    pyautogui.press("esc")
                    current_state = "START"
                    continue
