    ...
    try:
        ...
        while True:
            ...
            if current_state == "SCAN":
                ...
                if time.time() - SHADOW_VEIL_LASTTIME > 99999999999999999:
                    pyautogui.press("f3")
                    humanpause(0.5)
                    find_and_click_center(c_shadow_veil, tolerance=10)
                    SHADOW_VEIL_LASTTIME = time.time()
                    pyautogui.press("f1")

                if IsColourFound(c_winejug_inv, tolerance=1):
                    find_and_click_center(c_winejug_inv, tolerance=1)

                if IsColourFound(c_diamond_inv, tolerance=1):
                    find_and_click_center(c_diamond_inv, tolerance=1)

                if IsColourFound(c_gold_ore, tolerance=1):
                    find_and_click_center(c_gold_ore, tolerance=1)

                if IsColourFound(c_fire_orb_inv, tolerance=1):
                    find_and_click_center(c_fire_orb_inv, tolerance=1)

                if pursecount >= 50:
                    if find_and_click_center(c_coin_purse, tolerance=1):
                        pursecount = 0

                if hp <= 62:
                    find_and_click_center(c_shark_inv, tolerance=0)

                if IsColourFound(c_equip_amulet, tolerance=10):
                    if find_and_click_center(c_equip_amulet, tolerance=10):
                        humanpause(game_tick)

                if IsColourFound(c_ard_knight, tolerance=8):
                    if find_and_click_close_to(target_hex=c_ard_knight, proximity_hex=c_player_outline,
                                               tolerance=8, proximity_tolerance=2,
                                               clicks=10, interval=0.25):
                        pursecount += 2
