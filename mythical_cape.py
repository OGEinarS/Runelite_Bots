...
# Color definitions
c_teak_plank_inv = "886230"
c_cape_ready = "FFAAFFAA"
c_cape_menu = "7cb0d9"
c_cape_remove = "FFFF6B6B"
...
def print_player_stats(chisel=0,staminapot=0, bloodrunes=0, housetp=0, dense_e_b=0, dark_e_b=0, dark_essense=0,
                       hp=0, prayer=0, run=0, current_state=0, boss=0, teak=0):
...
def main():
    ...
    try:
        ...
        while True:
            ...
            teak_inv_count = count_items_full_screen(c_teak_plank_inv, 5, 50)
            ...
            if current_state == "MAIN_HANDLE":
                if teak_inv_count >= 6:
                    current_state = "CRAFT_CAPE"
                    continue

                if teak_inv_count < 6:
                    current_state = "BUTLER_HANDLE"
                    continue

            elif current_state == "CRAFT_CAPE":

                if IsColourFound(c_cape_remove, tolerance=1):
                    cape_status = "remove"
                if IsColourFound(c_cape_ready, tolerance=1):
                    cape_status = "build"

                if not IsColourFound(c_cape_remove, tolerance=1) and not IsColourFound(c_cape_ready, tolerance=1):
                    pyautogui.press("esc")

                if IsColourFound(c_idle, tolerance=25):
                    if cape_status == "remove":
                        if find_and_click_close_to(c_cape_remove, proximity_hex=c_player_outline, tolerance=1,proximity_tolerance=2):
                            humanpause(1)
                            pyautogui.press("1")
                            if teak_inv_count < 6:
                                current_state = "BUTLER_HANDLE"
                    ...
                    if cape_status == "build":
                        if find_and_click_close_to(c_cape_ready, proximity_hex=c_player_outline, tolerance=1,proximity_tolerance=2):
                            humanpause(1)
                            pyautogui.press("1")
                            if teak_inv_count < 6:
                                current_state = "BUTLER_HANDLE"
            ...
            elif current_state == "BUTLER_HANDLE":
                pyautogui.press("f10")
                humanpause(0.75)
                find_and_click_center(c_tab_house, tolerance=2)
                humanpause(1)
