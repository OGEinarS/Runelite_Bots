...
def main():
    ...
    try:
        ...
        while True:
            ...
            inv_gems = count_items_full_screen(COLOR_GEM_INV)

            print(f"Gems: {inv_gems}")


            if current_state == "HANDLE_MINING":
                if inv_gems >= 25:
                    current_state = "BANK_HAMDLE"
                    humanpause(.666)
                    continue

                if find_and_click_center(COLOR_GEM_ROCK, tolerance=1):
                    current_state = "HANDLE_MINING"
                    humanpause((.666)*10)
                    continue
            ...
            elif current_state == "BANK_HAMDLE":
                if find_and_click_center(COLOR_BANK_CHESTR, tolerance=1):
                    humanpause(4)
                    continue

                if find_and_click_center(COLOR_BANK_GEMS, tolerance=1):
                    humanpause(0.666)
                    pyautogui.press("esc")
                    current_state = "HANDLE_MINING"
                    continue
